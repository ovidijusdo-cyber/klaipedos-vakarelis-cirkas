...
const WHEEL_PRIZES = [
  ...
  "Padaryk linksmą, bet padorią pozą nuotraukai.",
  "Surask žmogų su įdomiausiu aksesuaru.",
  "Pakviesk žmogų pasikalbėti 2 minutes.",
  "Pasakyk komplimentą vakaro vedėjams.",
  "Paplok pirmam žmogui, kuris šoks.",
  "Pakviesk žmogų į mini šokį vietoje.",
  "Sugalvok trumpą pozityvų šūkį vakarui.",
  "Nusišypsok penkiems žmonėms.",
  "Pasidaryk nuotrauką su nauju pažįstamu.",
  "Paklausk žmogaus, iš kur jis atvyko.",
  "Pagirk skaniai atrodantį patiekalą.",
  "Pakviesk žmogų prisijungti prie bendros nuotraukos.",
  "Surask žmogų su panašia aprangos spalva.",
  "Pasakyk „gerai atrodai“ žmogui, kuris pasistengė dėl temos.",
  "Pakviesk žmogų prie stalo, jei jis stovi vienas.",
  "Sugalvok mini tostą be alkoholio.",
  "Pasiūlyk vieną dainą šokių sąrašui.",
  "Padėk kam nors pasidaryti nuotrauką.",
  "Paklausk žmogaus, kas jam labiausiai patinka vakare.",
  ...
  "Pakviesk žmogų į šokių aikštelę, kai gros kita daina.",
  "Sugalvok vieną gražų palinkėjimą vakarui.",
  ...
  "Padaryk nuotrauką su bent dviem žmonėmis.",
  "Paklausk žmogaus, kokia jo vakaro misija.",
  "Pasiūlyk žmogui atsisėsti prie jūsų stalo.",
  "Pasakyk komplimentą žmogui už gerą nuotaiką.",
  "Surask žmogų, kuris moka smagiai šokti.",
  "Pakviesk vieną žmogų sugalvoti bendrą pozą.",
  "Pasidaryk nuotrauką su žmogumi iš kito miesto.",
  "Paklausk žmogaus, kokia daina jį išjudintų.",
  "Pasveikink žmogų, kuris ką tik atvyko.",
  "Pakviesk žmogų kartu pasižiūrėti svečių lentos.",
  "Sugalvok vakaro „slaptą gerą darbą“ ir atlik jį.",
  "Pasakyk komplimentą už tvarkingą, padorų įvaizdį.",
  "Padaryk vieną grupinę asmenukę.",
  "Surask žmogų, kurio vardas prasideda ta pačia raide kaip tavo.",
  "Pakviesk žmogų kartu nueiti prie gėrimų stalo.",
  ...
  "Padaryk nuotrauką, kurioje visi rodo nykštį į viršų.",
  "Surask žmogų, kuris atnešė naminį patiekalą.",
  ...
  "Padėk kažkam rasti reikiamą žmogų ar vietą salėje.",
  "Paklausk žmogaus, kokį žaidimą jis norėtų išbandyti.",
  "Pakviesk žmogų kartu paploti vakaro vedėjams.",
  "Pasakyk vienam žmogui nuoširdų palinkėjimą.",
  "Surask žmogų, kuris šiandien atvyko su gera nuotaika.",
  "Pakviesk žmogų prie nuotraukų kampelio.",
  "Sugalvok mini apdovanojimą žmogui šalia savęs.",
  "Pasakyk komplimentą žmogui už draugiškumą.",
  "Atlik vieną mažą gerą darbą neprašomas.",
];
...
const initialMovieSeatReservations: MovieSeatReservation[] = [];
...
function normalizeMovieSeatReservations(items: MovieSeatReservation[]) {
  return items.flatMap((item) => {
    const seatId = String(item.seatId ?? "").trim().toUpperCase();
    const row = String(item.row ?? seatId.match(/^[A-Z]+/)?.[0] ?? "").trim().toUpperCase();
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function loadState() {
      try {
        ...
        const rawMovieSeatReservations = Array.isArray(parsed.movieSeatReservations) ? parsed.movieSeatReservations : [];
  ...
  function submitMovieReservation() {
    ...
    const incompleteSeat = movieSelectedSeats.find((seatId) => {
      const formValue = movieGuestForms[seatId];
      return !formValue?.firstName.trim() || !formValue.lastName.trim();
    });
    if (incompleteSeat) {
    ...
    const alreadyReservedSeat = movieSelectedSeats.find((seatId) => movieReservedSeatIds.has(seatId));
    if (alreadyReservedSeat) {
      setMovieNotice({ type: "warning", text: `Vieta ${alreadyReservedSeat} ką tik buvo užimta. Pasirink kitą vietą.` });
      setMovieSelectedSeats((previous) => previous.filter((seatId) => !movieReservedSeatIds.has(seatId)));
      return;
    }
    ...
    setMovieNotice({
      type: "success",
      text: `Rezervuota: ${movieSelectedSeats.join(", ")}. Mokėtina suma: ${movieSelectedTotal} €.`,
    });
  ...
  if (appMode === "movie") {
    return (
      <main className="page-shell movie-page">
        <div className="page-glow page-glow-left" />
        <div className="page-glow page-glow-right" />
        ...
        <section className="movie-hero">
          <div>
            ...
            <p>
              Pasirink norimas vietas salėje, prie kiekvienos vietos įrašyk žmogaus vardą ir pavardę, o sistema suskaičiuos mokėtiną sumą.
            </p>
          </div>
        ...
        <SectionCard
          ...
          description="Pilkos vietos jau užimtos. Pasirink kelias vietas, žemiau supildyk vardus ir patvirtink registraciją."
        >
          <div className="movie-layout">
            <div className="movie-seat-map" aria-label="Kino salės vietų planas">
              <div className="movie-screen">EKRANAS</div>
              <div className="movie-seat-legend">
                ...
                <span><i className="seat-sample reserved" /> Užimta</span>
              </div>
                ...
                {movieSeatRows.map((rowSeats) => (
                  <div className="movie-seat-row" key={rowSeats[0]?.row}>
                    ...
                    <div className="movie-row-seats">
                      {rowSeats.map((seat) => {
                        const reserved = movieReservedSeatIds.has(seat.id);
                        const selected = movieSelectedSeats.includes(seat.id);
                        return (
                          <button
                            aria-label={`Vieta ${seat.id}`}
                            className={reserved ? "movie-seat reserved" : selected ? "movie-seat selected" : "movie-seat"}
                            disabled={reserved}
                            key={seat.id}
                            type="button"
            ...
            <div className="movie-booking-panel">
              <div className="movie-total-card">
                <span>Pasirinkta vietų</span>
                <strong>{movieSelectedSeats.length}</strong>
                <p>Mokėtina suma: {movieSelectedTotal} €</p>
              </div>
        ...
        <SectionCard title="Rezervuotos vietos" description="Čia matosi jau užimtos vietos, kad būtų aišku kiek žmonių užsiregistravo.">
          {movieSeatReservations.length === 0 ? (
            ...
            <div className="movie-reservation-list">
              {movieSeatReservations
                ...
                .map((reservation) => (
                  <div className="movie-reservation-row" key={reservation.id}>
      ...
      <main className="page-shell">
        ...
        <div className="hero-copy">
          ...
          <div className="stats-grid">
            ...
            <StatCard label={`Iki ${CHILD_AGE_LIMIT} m.`} value={String(totalChildren)} tone="accent" />
